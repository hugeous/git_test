...
<form name="frmWrite" id="frmWrite" action="article_ps.php" method="post" enctype="multipart/form-data">
    ...
    <table class="table table-cols" id="board-table">
        ...
        <?php } ?>
		<?php
			 if ($bdWrite['cfg']['bdId'] == 'goodsreview') { ?>
			<tr>
                <th>메인 노출 여부</th>
                <td>
                    <input type="radio" name="reviewMainFl" value="1" class="radio" id="reviewMainFl1" <?php if ($bdWrite['data']['reviewMainFl'] == '1') echo 'checked' ?> >노출함&nbsp;
					<input type="radio" name="reviewMainFl" value="0" class="radio" id="reviewMainFl1" <?php if ($bdWrite['data']['reviewMainFl'] == '0') echo 'checked' ?> >노출안함&nbsp;
                </td>
            </tr>
        <?php } ?>
        <?php
