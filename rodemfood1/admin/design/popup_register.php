<form id="frmPopup" name="frmPopup" method="post" action="./popup_ps.php" enctype="multipart/form-data">
    ...
    <table class="table table-cols">
		...
		<tr>
            <th>팝업 컨텐츠 종류</th>
            <td>
                <label class="radio-inline"><input type="radio" name="contentsType" value="0" <?php if($data['contentsType'] == "0") { ?>checked<?php } ?> required /> 일반</label>
				<label class="radio-inline"><input type="radio" name="contentsType" value="1" <?php if($data['contentsType'] == "1") { ?>checked<?php } ?> required /> 움직이는 배너</label>
				<label class="radio-inline"><input type="radio" name="contentsType" value="2" <?php if($data['contentsType'] == "2") { ?>checked<?php } ?> required /> 일반 배너</label>
            </td>
        </tr>
		<tr>
            <th>배너 코드</th>
            <td>
				<div class="form-inline">
				PC : <input type="text" name="bannerCode" value="<?php echo $data['bannerCode']; ?>" class="form-control width-lg" placeholder="배너코드" required>
				/ Mobile : <input type="text" name="mobileBannerCode" value="<?php echo $data['mobileBannerCode']; ?>" class="form-control width-lg" placeholder="배너코드" required>
                <p class="notice-info">
					움직이는 배너 : 왼쪽 메뉴 중 움직이는 배너 관리에서 배너 등록 후 코드 등록<br />
					일반 배너 : 왼쪽 메뉴 중 배너 관리에서 배너 등록 후 코드 등록
				</p>
				</div>
            </td>
        </tr>
